...
def _load_custom_xp(path: str):
    """Load an XP file as (pixels, w, h) row-major RGBA list."""
    import gzip, struct
    from pathlib import Path
    from scripts.launcher_lib.banner import MAGENTA_BG, _cell_pixel_color
    p = Path(path)
    if not p.exists():
        return None, 0, 0
    with open(p, 'rb') as f:
        raw = gzip.decompress(f.read())
    offset = 8  # skip version + layer_count
    w = struct.unpack_from('<i', raw, offset)[0]; offset += 4
    h = struct.unpack_from('<i', raw, offset)[0]; offset += 4
    # cells are column-major in XP
    grid = [[None]*w for _ in range(h)]
    for x in range(w):
        for y in range(h):
            glyph = struct.unpack_from('<I', raw, offset)[0]; offset += 4
            fg = tuple(raw[offset:offset+3]); offset += 3
            bg = tuple(raw[offset:offset+3]); offset += 3
            c = _cell_pixel_color(glyph, fg, bg)
            grid[y][x] = (*c, 0 if c == MAGENTA_BG else 255)
    return [px for row in grid for px in row], w, h


_CUSTOM_BANNER = '/tmp/asciicker_logo_red.xp'
...
def main():
    ...
    pixels, src_w, src_h = _load_custom_xp(_CUSTOM_BANNER)
    if not pixels:
