"""B-hellish: Scrolling hue sweep restricted to the red-orange-yellow band.

Hue maps to [0.95 → 0.12] wrapping through 0.0 — deep red through
orange to warm yellow. Saturation forced to minimum 0.85 for vivid fire.

Loops until 'q' or Ctrl-C. Any other key restarts.
Resize: restarts sweep at new size.

Run: python3 testing/anim_B_hellish.py
"""
...
# Hue band: 0.95 (deep red) → 0.12 (warm yellow), span = 0.17, wraps through 0
_BAND_START = 0.95
_BAND_SPAN  = 0.17


def _hellish(r, g, b, a, gx, gy, gw, gh, _p=0.0):
    if a < 16:
        return 0, 0, 0, 0
    _, s, v = colorsys.rgb_to_hsv(r / 255, g / 255, b / 255)
    t = (gx / gw + _p) % 1.0
    h = (_BAND_START + t * _BAND_SPAN) % 1.0
    s = max(s, 0.85)
    v = max(v, 0.25)
            ...
            while True:
                ...
                while frame <= FRAMES:
                    ...
                    def color_fn(r, g, b, a, gx, gy, gw, gh, _p=_p):
                        return _hellish(r, g, b, a, gx, gy, gw, gh, _p)
