...
# Palette-safe RGB values for Asciicker terminal rendering
SAFE_LEVELS = {0, 51, 102, 153, 204, 255}

def validate_colors_for_export(mesh, obj):
    """Check vertex colors and return warnings for export"""
    warnings = []
    ...
    if dither_count > 0:
        ...
        warnings.append(f"{dither_count}/{total} ({pct:.0f}%) colors will use dithering")

    # Check for active vertex group (used for alpha/collision)
    if not obj.vertex_groups.active:
        warnings.append("No vertex group for collision - all faces will be solid")
...
def save(
    ...
    # Validate colors before export
    if use_colors:
        warnings = validate_colors_for_export(mesh, obj)
        for w in warnings:
