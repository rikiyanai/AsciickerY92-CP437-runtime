// =============================================================================
// platform/backend/host_sdl2.cpp — SDL2 Runtime Host Adapter
// =============================================================================
//
// PLATFORM TARGET:
// Windows, Linux, macOS — any OS with SDL2 support
// Dependencies: SDL2 (windowing, input, gamepad), SDL_opengl.h (OpenGL context)
//
// WHY SDL2:
// - Cross-platform: single codebase for Windows/Linux/macOS
// - Gamepad support: SDL_GameController API with built-in community mappings
// - Simpler than native: no platform-specific event loop complexity
//
// OS-SPECIFIC INTEGRATION:
// - Windows: #include <SDL.h> (from SDL2/ prefix on Unix)
// - Unix:    #include <SDL2/SDL.h>
// - OpenGL:  SDL_GL_CreateContext(), SDL_GL_MakeCurrent(), SDL_GL_SwapWindow()
// - Timing:  clock_gettime(CLOCK_MONOTONIC) on Unix, QueryPerformanceCounter on Windows
//
// EVENT LOOP STRUCTURE:
// a3dLoop() → while(running) → SDL_PollEvent() → translate SDL events → PlatformInterface callbacks
//
// SDL Event Types Handled:
// - SDL_QUIT:                      close all windows, exit loop
// - SDL_WINDOWEVENT:               resize, focus, close events
// - SDL_KEYDOWN/KEYUP:             translate SDL_Scancode → KeyInfo enum → keyb_key()
// - SDL_TEXTINPUT:                 UTF-8 → wchar_t → keyb_char()
// - SDL_MOUSEMOTION:               mouse()
// - SDL_MOUSEBUTTONDOWN/UP:        LEFT_DN, RIGHT_DN, MIDDLE_DN → mouse()
// - SDL_MOUSEWHEEL:                WHEEL_UP, WHEEL_DN → mouse()
// - SDL_CONTROLLERDEVICEADDED:     detect gamepad, call gpad_mount()
// - SDL_CONTROLLERDEVICEREMOVED:   disconnect gamepad, call gpad_unmount()
// - SDL_CONTROLLERBUTTONDOWN/UP:   gpad_button()
// - SDL_CONTROLLERAXISMOTION:      gpad_axis()
//
// GAMEPAD INTEGRATION:
// - SDL_GameControllerOpen(0) → SDL_GameController* handle
// - SDL_GameControllerGetButton(gpad, SDL_CONTROLLER_BUTTON_A) → button state
// - SDL_GameControllerGetAxis(gpad, SDL_CONTROLLER_AXIS_LEFTX) → axis value
// - gpad_mount() called with SDL controller name, axes (6), buttons (15), mapping
// - gpad_unmount() called on disconnect
// - WHY SDL gamepad: Native X11/Win32 backends lack gamepad APIs, SDL provides
//   community-sourced mappings for hundreds of controller types
//
// KEY DATA STRUCTURES:
// - A3D_WND:        SDL_Window* win, SDL_GLContext rc, linked list (wnd_head/wnd_tail)
// - GlobalSDL:      SDL_GameController* gamepad, SDL_Init/Quit lifecycle
// - wnd_head/tail:  Global linked list of all open windows
// - A3D2SDL[]:      KeyInfo → SDL_Scancode translation table (for a3dGetKeyb)
// - SDL2A3D[]:      SDL_Scancode → KeyInfo translation table (for event loop)
//
// KEY FUNCTIONS:
// - a3dOpen():      SDL_CreateWindow(), SDL_GL_CreateContext(), attach to linked list
// - a3dClose():     SDL_DestroyWindow(), SDL_GL_DeleteContext(), unlink from list
// - a3dLoop():      Main event loop (SDL_PollEvent), dispatches to PlatformInterface
// - a3dGetTime():   clock_gettime(CLOCK_MONOTONIC) on Unix, QueryPerformanceCounter on Windows
//
// INTEGRATION POINTS:
// - runtime_host.h  — RuntimeHost seam (window/input/time/files)
// - game.cpp        — Provides PlatformInterface callbacks (OnKeyb, OnMouse, OnRender)
// - gamepad.cpp     — Receives gpad_button/gpad_axis events via LoopInterface
//
// KEY FILES:
// - host_sdl2.cpp   — This file: SDL2 backend implementation
// - runtime_host.h  — Contract composition header
// - game.cpp        — Game engine callbacks
// - gamepad.cpp     — Gamepad mapping UI and event processing
// =============================================================================
//
// BACKEND CONFORMANCE:
// This file is compiled ONLY into SDL2 targets.  No #ifdef USE_SDL guard.
// The makefile selects exactly one host adapter per binary (FL-2910).
// =============================================================================
...
// [FL-3851] MCP-triggered frame capture path. Set by CAPTURE_FRAME command in asciiid.cpp.
// When non-empty, host_sdl2 captures the next rendered frame to this path then clears it.
char g_mcp_capture_path[1024] = {0};

// [FL-3851] MCP-triggered clean frame capture directory. Set by CAPTURE_CLEAN_FRAME command.
// When non-empty, asciiid.cpp captures a clean frame (no UI) to frame.png + frame.camera.json
// in this directory, then clears the path.
char g_mcp_clean_capture_dir[1024] = {0};

// [FL-4131] MCP-triggered UI frame capture directory. Set by CAPTURE_UI_FRAME command.
// When non-empty, asciiid.cpp captures the full composited framebuffer (game + ImGui panels)
// to ui_frame.png in this directory after ImGui_ImplOpenGL3_RenderDrawData, then clears the path.
char g_mcp_ui_capture_dir[1024] = {0};

// When asciiid MCP mode is active, disable VSync so macOS App Nap cannot
// throttle the render loop below interactive rate. This file also links into
// the game target, so provide a weak default that asciiid.cpp overrides.
bool g_mcp_mode __attribute__((weak)) = false;
bool g_asciiid_editor_idle_yield __attribute__((weak)) = false;

static bool SdlMcpModeEnabled()
{
	return g_mcp_mode;
}

static bool SdlAsciiidEditorIdleYieldEnabled()
{
	return g_asciiid_editor_idle_yield;
}
...
extern "C" int a3dQueueSdlMouseDragProbe(int x0, int y0, int x1, int y1, int steps)
{
	if (!wnd_head || !wnd_head->win)
		return 0;
	if (steps < 1)
		steps = 1;
	if (steps > 20000)
	...
	const uint32_t window_id = SDL_GetWindowID(wnd_head->win);
	int queued = 0;
	...
	{
		int x = x0 + (int)(((int64_t)x1 - (int64_t)x0) * i / steps);
		int y = y0 + (int)(((int64_t)y1 - (int64_t)y0) * i / steps);
		memset(&ev, 0, sizeof(ev));
		ev.type = SDL_MOUSEMOTION;
		ev.motion.windowID = window_id;
		ev.motion.x = x;
		ev.motion.y = y;
		ev.motion.xrel = x - prev_x;
		ev.motion.yrel = y - prev_y;
...
// Tooling hook: allow game code (observe-render / headless capture) to request a
// graceful shutdown by queueing SDL_QUIT. This ensures the normal cleanup path
// runs (including owned local server teardown) instead of _Exit() leaking child
// processes.
...
A3D_WND* a3dOpen(const PlatformInterface* pi, const GraphicsDesc* gd, A3D_WND* share)
{
	if ((SDL_WasInit(SDL_INIT_VIDEO) & SDL_INIT_VIDEO) == 0)
	{
		...
		{
			fprintf(stderr, "[SDL] ERROR: SDL_InitSubSystem(SDL_INIT_VIDEO) failed: %s\n", SDL_GetError());
			return 0;
		}
	}
	...
	*/
	...
	// [PLATFORM:SDL] Window creation with OpenGL context
	...
	if (!wnd->rc)
	{
		fprintf(stderr, "[SDL] ERROR: SDL_GL_CreateContext failed: %s\n", SDL_GetError());
		return 0;
	}
	if (SDL_GL_MakeCurrent(wnd->win, wnd->rc) != 0)
	{
		fprintf(stderr, "[SDL] ERROR: SDL_GL_MakeCurrent failed: %s\n", SDL_GetError());
...
void a3dSetFocus(A3D_WND* wnd)
{
	if (!wnd || !wnd->win)
		return;
	SDL_RaiseWindow(wnd->win);
	SDL_SetWindowInputFocus(wnd->win);
}
...
bool a3dGetFocus(A3D_WND* wnd)
{
	if (!wnd || !wnd->win)
		return false;
	return (SDL_GetWindowFlags(wnd->win) & SDL_WINDOW_INPUT_FOCUS) != 0;
}
...
// [DATA-CONTRACT:KEYINFO] KeyInfo → SDL_Scancode translation table
// Maps platform.h KeyInfo enum (A3D_BACKSPACE, A3D_TAB, etc.) to SDL scancodes.
// Used by a3dGetKeyb() to query keyboard state via SDL_GetKeyboardState().
// If platform.h KeyInfo enum changes, this table MUST be updated to match.
int A3D2SDL[] =
{
...
// [DATA-CONTRACT:KEYINFO] SDL_Scancode → KeyInfo translation table
// Maps SDL scancodes to platform.h KeyInfo enum for event loop translation.
// WHY [128]: Covers SDL_SCANCODE_* range (0-127), larger scancodes handled separately.
// If platform.h KeyInfo enum changes, this table MUST be updated to match.
KeyInfo SDL2A3D[/*128*/] =
{
	A3D_NONE,
...
{
	// [PLATFORM:SDL] Enable gamepad event reporting
	...
	bool Running = true;
	uint64_t fl3714_last_loop_log = a3dGetTime();
	while (Running)
	{
		uint64_t fl3714_loop_t0 = a3dGetTime();
		int fl3714_events = 0;
		int fl3714_rendered = 0;
		// [FLOW:INPUT] Stage 1 → Stage 2: SDL event poll and translation
		SDL_Event Event;
		while (SDL_PollEvent(&Event))
		{
			...
			{
				case SDL_QUIT:
					// In MCP mode macOS sends SDL_QUIT to the background
					// SDL window; ignore it so the daemon stays alive.
					if (!g_mcp_mode) Running = 0;
					break;

				CASE(SDL_CONTROLLERDEVICEADDED)
				{
					// [PLATFORM:SDL] Gamepad detection and mounting
				...
				{
					// [FLOW:INPUT] Gamepad axis: SDL controller axis → gpad_axis()
				...
				{
					// [FLOW:INPUT] Gamepad button: SDL controller button → gpad_button()
				...
				*/
				...
				case SDL_TEXTINPUT:
				{
					// [FLOW:INPUT] Text input: UTF-8 → wchar_t
				...
				case SDL_WINDOWEVENT:
				{
					...
					switch (ev->event)
					{
						case SDL_WINDOWEVENT_CLOSE:
							...
							// If all windows are gone, exit even in MCP mode.
							if (!wnd_head)
								Running = 0;
							break;
				...
				case SDL_KEYUP:
				{
					// [FLOW:INPUT] Keyboard event: SDL_Scancode → KeyInfo enum
				...
				{
					// [FLOW:INPUT] Mouse movement: SDL motion event → mouse()
					...
					// FL-3714: use the state carried by this motion event. Querying
					// global mouse state loses held-button flags for queued/proof events.
					int b = ev->state;
				...
				{
					// [FLOW:INPUT] Mouse button: SDL button index → MouseInfo enum (LEFT_DN, RIGHT_DN, etc.)
		...
		if (swap)
		{
				// [FL-3838/FL-3851] Framebuffer capture for automated visual verification.
				// Two triggers: (1) ASCIICKER_CAPTURE_FRAME env var on startup (one-shot after 5 frames)
				//                (2) g_mcp_capture_path set by CAPTURE_FRAME MCP command (re-triggerable)
				{
					static bool startup_capture_done = false;
					static int startup_capture_delay = 0;

					const char* cap = nullptr;
					int delay_ok = 1;

					// MCP-triggered capture (immediate, re-triggerable)
					if (g_mcp_capture_path[0])
					{
						static int mcp_capture_delay = 0;
						mcp_capture_delay++;
						if (mcp_capture_delay >= 3) // wait 3 frames for camera to settle
						{
							cap = g_mcp_capture_path;
							mcp_capture_delay = 0;
						}
						else
							delay_ok = 0;
					}
					// Startup env var capture (one-shot)
					else if (!startup_capture_done)
					{
						const char* env_cap = getenv("ASCIICKER_CAPTURE_FRAME");
						if (env_cap && *env_cap)
						{
							startup_capture_delay++;
							if (startup_capture_delay >= 5)
							{
								cap = env_cap;
					...
					{
						int w, h;
						SDL_GL_GetDrawableSize(swap->win, &w, &h);
						...
						if (px)
						{
							...
							if (f)
							{
								...
								for (int row = h - 1; row >= 0; row--)
									fwrite(px + row * w * 3, 1, w * 3, f);
								fclose(f);
								printf("[FL-3851] FRAME_CAPTURE: %dx%d -> %s\n", w, h, cap);
								fflush(stdout);
							}
							free(px);
						}
						if (g_mcp_capture_path[0])
							g_mcp_capture_path[0] = 0; // clear MCP trigger
					}
				}

				const bool mcp_mode = SdlMcpModeEnabled();
				SDL_GL_SetSwapInterval(mcp_mode ? 0 : 1);
				SDL_GL_SwapWindow(swap->win);
				if (mcp_mode) SDL_Delay(1); // yield CPU; VSync disabled to prevent App Nap throttle
				else if (SdlAsciiidEditorIdleYieldEnabled() && fl3714_events == 0)
					SDL_Delay(16);
			}

		uint64_t fl3714_now = a3dGetTime();
		uint64_t fl3714_loop_us = fl3714_now - fl3714_loop_t0;
		if (fl3714_loop_us > 250000 || fl3714_now - fl3714_last_loop_log > 2000000)
		{
			printf("[SDL] [FL-3714] loop_ms=%.3f events=%d rendered=%d swapped=%d windows=%d\n",
				(double)fl3714_loop_us / 1000.0,
				fl3714_events,
				fl3714_rendered,
				swap ? 1 : 0,
				wnd_head ? 1 : 0);
			fflush(stdout);
			fl3714_last_loop_log = fl3714_now;
		}
	}
}
...
// [PLATFORM:TIMING] Monotonic microsecond clock (Unix)
//
// WHY CLOCK_MONOTONIC:
// - Monotonic: never goes backwards (unaffected by NTP adjustments, leap seconds)
// - High precision: nanosecond resolution (converted to microseconds)
// - Wraps every 584542 years (uint64_t microseconds: 2^64 / 1000000 / 60 / 60 / 24 / 365)
//
// WHY not CLOCK_REALTIME:
// - CLOCK_REALTIME can jump backwards when system clock is adjusted
// - Game timing must never go backwards (causes physics glitches, negative deltas)
//
// Conversion: ts.tv_sec * 1000000 (seconds → microseconds) + ts.tv_nsec / 1000 (nanoseconds → microseconds)
uint64_t a3dGetTime()
{
	static timespec ts;
...
// [PLATFORM:TIMING] High-precision performance counter (Windows)
//
// WHY QueryPerformanceCounter:
// - High precision: microsecond or better (frequency typically 10 MHz on modern CPUs)
// - Monotonic: never goes backwards (guaranteed by Windows)
// - Wraps every 584542 years (uint64_t microseconds: 2^64 / 1000000 / 60 / 60 / 24 / 365)
//
// WHY not GetTickCount64:
// - GetTickCount64 has only millisecond precision (insufficient for physics/animations)
// - QueryPerformanceCounter provides sub-microsecond precision on modern CPUs
//
// Conversion: (counter * 1000000) / frequency = microseconds
uint64_t a3dGetTime()
{
	struct SafeTimer
	{
