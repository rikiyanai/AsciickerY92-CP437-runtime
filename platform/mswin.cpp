// =============================================================================
// Win32/WGL Platform Backend — Windows Native Windowing & Input
// =============================================================================
//
// PLATFORM TARGET:
// Windows 7 SP1 and later (Vista+ with Platform Update)
// Compile flag: _WIN32 && !defined(USE_SDL)
// Dependencies: Windows.h (Win32 API), OpenGL32.lib (WGL), wglext.h (WGL extensions)
//
// WHY WIN32 NATIVE (not SDL):
// - Lower latency: direct Win32 message handling (no SDL translation layer)
// - Better Windows integration: taskbar, DPI awareness, native dialogs
// - Finer control: direct access to WM_* messages, window styles, window procedures
// - Smaller binary: no SDL2 dependency
//
// OS-SPECIFIC INTEGRATION:
// - Win32: CreateWindowEx(), ShowWindow(), GetMessage(), DispatchMessage()
// - WGL: wglCreateContext(), wglMakeCurrent(), SwapBuffers()
// - Timing: QueryPerformanceCounter(), QueryPerformanceFrequency()
// - Gamepad: UNSUPPORTED (no native Win32 gamepad API, use SDL backend instead)
//
// EVENT LOOP STRUCTURE:
// a3dLoop() → while(GetMessage) → TranslateMessage → DispatchMessage → WndProc → PlatformInterface callbacks
//
// WM_* Message Types Handled:
// - WM_CLOSE:         close window
// - WM_SIZE:          resize event → resize()
// - WM_PAINT:         redraw event → render()
// - WM_KEYDOWN/UP:    Virtual key code → KeyInfo enum → keyb_key()
// - WM_CHAR:          UTF-16 → wchar_t → keyb_char()
// - WM_LBUTTONDOWN:   LEFT_DN → mouse()
// - WM_RBUTTONDOWN:   RIGHT_DN → mouse()
// - WM_MBUTTONDOWN:   MIDDLE_DN → mouse()
// - WM_MOUSEMOVE:     mouse(MOVE)
// - WM_MOUSEWHEEL:    WHEEL_UP, WHEEL_DN → mouse()
// - WM_SETFOCUS/KILLFOCUS: keyb_focus()
//
// KEY DATA STRUCTURES:
// - A3D_WND:        HWND hwnd, HDC dc, HGLRC rc, linked list (wnd_head/wnd_tail)
// - wnd_head/tail:  Global linked list of all open windows
// - ki_to_vk[256]:  KeyInfo → VK_* virtual key code translation table
//
// KEY FUNCTIONS:
// - a3dOpen():      CreateWindowEx(), wglCreateContext(), attach to linked list
// - a3dClose():     DestroyWindow(), wglDeleteContext(), unlink from list
// - a3dLoop():      Main event loop (GetMessage), dispatches to PlatformInterface
// - a3dGetTime():   QueryPerformanceCounter() → microseconds
// - WndProc():      Window procedure (dispatches WM_* messages to PlatformInterface)
//
// INTEGRATION POINTS:
...
//
// KEY FILES:
// - mswin.cpp       — This file: Win32/WGL backend implementation
...
// [DATA-CONTRACT:KEYINFO] KeyInfo → VK_* translation table
// Maps platform.h KeyInfo enum to Windows virtual key codes.
// Used by a3dGetKeyb() to query keyboard state via GetAsyncKeyState().
...
// [DATA-CONTRACT:KEYINFO] VK_* → KeyInfo translation table
// Maps Windows virtual key codes to platform.h KeyInfo enum for event loop translation.
// WHY [256]: Covers full VK_* range (0-255).
// If platform.h KeyInfo enum changes, this table MUST be updated to match.
...
// [FLOW:INPUT] Stage 1 → Stage 2: Win32 message dispatch
// Window procedure translates WM_* messages to PlatformInterface callbacks
LRESULT WINAPI a3dWndProc(HWND h, UINT m, WPARAM w, LPARAM l)
{
	...
	if (m == WM_CREATE)
	{
		...
		// [PLATFORM:WIN32] High-resolution timing setup
		// Initialize QueryPerformanceFrequency/Counter on first window creation
		if (!wnd_head)
		{
	...
	switch (m)
	{
		...
		case WM_CHAR:
			// [FLOW:INPUT] Text input: UTF-16 wParam → wchar_t
		...
		case WM_KEYDOWN:
		case WM_KEYUP:
			// [FLOW:INPUT] Keyboard event: Virtual key code → KeyInfo enum
			// [DATA-CONTRACT:KEYINFO] vk_to_ki[] maps VK_* codes to platform.h KeyInfo enum
		...
		*/
		...
		// [FLOW:INPUT] Mouse button: WM_*BUTTONDOWN/UP → MouseInfo enum (LEFT_DN, RIGHT_DN, etc.)
		case WM_LBUTTONDOWN:
...
A3D_WND* a3dOpen(const PlatformInterface* pi, const GraphicsDesc* gd, A3D_WND* share)
{
	...
	// [PLATFORM:WIN32] Window class registration
	// WHY: Win32 requires window class registration before CreateWindow (not needed in SDL/X11)
	WNDCLASS wc;
	wc.style = 0;
...
// NOTE: coarse_perf/coarse_micro refreshed every 60 seconds to prevent overflow
uint64_t a3dGetTime()
{
	struct SafeTimer
	{
