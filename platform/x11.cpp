// =============================================================================
// X11/GLX Platform Backend — Unix Native Windowing & Input
// =============================================================================
//
// PLATFORM TARGET:
// Linux, BSD, macOS (with XQuartz) — any Unix with X11/GLX support
// Compile flag: (defined(__linux__) || defined(__APPLE__)) && !defined(USE_SDL)
// Dependencies: X11, GLX, Xinerama (multi-monitor), pthread (threading), libutil (pty)
//
// WHY X11 NATIVE (not SDL):
// - Lower latency: direct X11 event handling (no SDL translation layer)
// - Better terminal integration: pty support for embedded terminal (a3dOpenPty)
// - Finer control: direct access to X11 atoms, window properties, XIM input methods
// - Smaller binary: no SDL2 dependency
//
// OS-SPECIFIC INTEGRATION:
// - X11: XOpenDisplay(), XCreateWindow(), XMapWindow(), XNextEvent()
// - GLX: glXCreateContext(), glXMakeCurrent(), glXSwapBuffers()
// - Input Method: XOpenIM(), XCreateIC() for international text input (i18n)
// - Clipboard: XConvertSelection(), XGetWindowProperty() for copy/paste
// - Gamepad: UNSUPPORTED (no native X11 gamepad API, use SDL backend instead)
//
// EVENT LOOP STRUCTURE:
// a3dLoop() → while(running) → XPending() / poll() → XNextEvent() → translate X events → PlatformInterface callbacks
//
// X11 Event Types Handled:
// - ClientMessage (WM_DELETE_WINDOW): close window
// - ConfigureNotify:      resize event → resize()
// - Expose:               redraw event → render()
// - KeyPress/KeyRelease:  XLookupKeysym → KeyInfo enum → keyb_key()
// - KeyPress:             XmbLookupString → wchar_t → keyb_char() (via XIM/XIC)
// - ButtonPress/Release:  X11 button codes → MouseInfo (LEFT_DN, RIGHT_DN, etc.) → mouse()
// - MotionNotify:         mouse x,y → mouse(MOVE)
// - FocusIn/FocusOut:     keyb_focus()
//
// KEY DATA STRUCTURES:
// - A3D_WND:        Window win, GLXContext rc, XIM im, XIC ic, linked list (wnd_head/wnd_tail)
// - Display* dpy:   Global X11 display connection (opened once in a3dOpen)
// - wnd_head/tail:  Global linked list of all open windows
// - A3D_PTY:        Pseudo-terminal for embedded terminal support (not in SDL/Win32 backends)
//
// KEY FUNCTIONS:
// - a3dOpen():      XCreateWindow(), glXCreateContext(), XOpenIM(), attach to linked list
// - a3dClose():     XDestroyWindow(), glXDestroyContext(), unlink from list
// - a3dLoop():      Main event loop (XNextEvent), dispatches to PlatformInterface
// - a3dGetTime():   clock_gettime(CLOCK_MONOTONIC)
// - a3dOpenPty():   openpty(), forkpty() for terminal embedding (X11 ONLY)
//
// INTEGRATION POINTS:
...
// - terminal.cpp    — Uses a3dOpenPty() for embedded terminal (X11 ONLY)
//
// KEY FILES:
// - x11.cpp         — This file: X11/GLX backend implementation
// - platform.h      — Contract definition (PlatformInterface, KeyInfo, MouseInfo)
// - game.cpp        — Game engine callbacks
// - term.cpp        — Terminal rendering and pty integration
// =============================================================================
...
A3D_WND* a3dOpen(const PlatformInterface* pi, const GraphicsDesc* gd, A3D_WND* share)
{
	...
	[Desktop Entry]
			...
			Icon=/home/user/asciiid/assets/icons/app.png
			Type=Application
	...
	*/
	...
	XIC ic = 0;
	// [PLATFORM:X11] Input method for international text (X11 ONLY feature)
	// WHY XIM/XIC: Enables international keyboard input (Chinese, Japanese, Korean, etc.)
	// via X Input Method protocol. Not needed in SDL backend (SDL_TEXTINPUT handles this).
	if (im_ok)
	{
...
void a3dLoop()
{
	...
	while (wnd_head)
	{
		// [FLOW:INPUT] Stage 1 → Stage 2: X11 event poll and translation
		while (XPending(dpy))
		{
			XNextEvent(dpy, &xev);
			...
			{
				// [FLOW:INPUT] Keyboard event: X11 keycode → KeyInfo enum
				// [DATA-CONTRACT:KEYINFO] kc_to_ki[] maps X11 keycodes to platform.h KeyInfo enum
				...
				// [FLOW:INPUT] Text input: Xutf8LookupString → wchar_t (via XIM/XIC)
				if (wnd->ic)
				{
					int count = 0;
					KeySym keysym = 0;
			...
			{
				// [FLOW:INPUT] Mouse button: X11 button codes → MouseInfo enum (LEFT_DN, RIGHT_DN, etc.)
				int state = 0;
...
// Threading removed: a3dCreateThread/a3dCreateMutex had zero callers.
// Use THREAD_CREATE/MUTEX_CREATE from server/network.h instead.
...
A3D_PTY* a3dOpenPty(int w, int h, const char* path, char* const argv[], char* const envp[])
{
    ...
    // [PLATFORM:X11] Pseudo-terminal allocation (X11 ONLY feature)
    // WHY openpty: Enables embedded terminal support for in-game terminal emulator.
    // Not available in SDL/Win32 backends (Unix pty API only).
    struct winsize ws;
    ws.ws_col = w;
    ws.ws_row = h;
    ws.ws_xpixel=0;
