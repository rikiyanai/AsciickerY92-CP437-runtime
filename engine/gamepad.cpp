// =============================================================================
// Gamepad Mapping Configuration — Visual Drag-Drop Mapping UI
// =============================================================================
//
// PURPOSE:
// Implements a visual gamepad configuration UI that allows players to customize
// button/axis mappings by dragging input slots onto output positions. The system
// supports two gamepad layouts (Xbox, PS5), maintains inverse lookup tables for
// efficient event routing, and persists mapping to disk.
//
// MAPPING SYSTEM ARCHITECTURE:
//
// INPUT SPACE (256 indices):
// - Gamepad can have up to 256 buttons/axes (implementation-defined)
// - SDL backend reports: 15 buttons (A,B,X,Y,LB,RB,etc), 6 axes (LX,LY,RX,RY,LT,RT)
// - gamepad_button[256]: raw button states (0 = released, 32767 = pressed)
// - gamepad_axis[256]:   raw axis values (-32768 to +32767)
// - Axes use TWO indices: negative half (2*a) and positive half (2*a+1)
//
// OUTPUT SPACE (21 indices):
// - 6 axes (0-5):    Left stick L/R/U/D, Right stick L/R/U/D (actually 12 half-axes)
// - 15 buttons (6-20): A, B, X, Y, E, G, F, L, R, Ls, Rs, Du, Dd, Dl, Dr
// - gamepad_axis_output[6]:    mapped axis states (after applying gamepad_mapping[])
// - gamepad_button_output[15]: mapped button states
//
// OUTPUT INDEX TABLE:
//   Index  Type    Name    Description
//   0-1    Axis    Ll,Lr   Left stick left/right
//   2-3    Axis    Lu,Ld   Left stick up/down
//   4-5    Axis    Rl,Rr   Right stick left/right
//   (6-7   Axis    Ru,Rd   Right stick up/down — implemented as half-axes in UI)
//   (8-11  Axis    Lt,Rt   Triggers left/right — implemented as half-axes in UI)
...
// - button_mapping[15]:        Inverse: Output button → list of input indices (0xFF terminated)
// - axis_mapping[6]:           Inverse: Output axis → list of input indices (0xFF terminated)
//
// WHY INVERSE TABLES:
// Inverse tables enable efficient "which inputs map to this output?" queries.
// Example: button_mapping[0] = {5, 12, 0xFF} means input buttons 5 and 12 both map to output A.
// Use case: Visual UI highlights ALL input slots that map to selected output.
//
// INVERSE TABLE REBUILD PROTOCOL:
// Inverse tables are rebuilt every time gamepad_mapping[] changes (drag-drop, load from disk):
// 1. Clear all existing button_mapping[] and axis_mapping[] arrays (free + NULL)
// 2. Count length needed for each output (scan gamepad_mapping[])
// 3. Allocate new arrays with +1 for 0xFF sentinel
// 4. Populate arrays by iterating gamepad_mapping[] again
// 5. Terminate each array with 0xFF sentinel
//
// WHY rebuild from scratch (not incremental):
// - Simpler: No need to track old→new delta, just regenerate entire structure
// - Safer: No risk of stale pointers or incomplete updates
// - Fast enough: 256 iterations is O(1) for human interaction (drag-drop)
//
// VISUAL CONFIGURATION UI:
//
// LAYOUT:
// - Gamepad sprite (50×27 cells): Xbox or PS5 controller visual
// - Output positions (gamepad_button_xy[], gamepad_half_axis_xy[]): fixed positions on sprite
// - Input slots: draggable UI elements representing unmapped inputs
// - Drag-drop: drag input slot onto output position to create mapping
//
// DRAG-DROP STATE MACHINE:
// - gamepad_contact = -1:      No contact (mouse/touch not pressed)
// - gamepad_contact = 0:       Mouse contact (left button down)
// - gamepad_contact = 1+:      Touch contact (touch ID)
// - gamepad_contact_from[2]:   Drag start position (cell coordinates)
// - gamepad_contact_pos[2]:    Current drag position (updated on move)
// - gamepad_contact_output:    Output index being dragged (0xFF = input slot)
// - gamepad_contact_ui:        UI element being dragged (0=clear, 1=init, 2=quit, etc.)
//
// XBOX VS PS5 LAYOUTS:
// - gamepad_assembly = 0: Xbox layout (A/B/X/Y diamond on right, D-pad on left)
// - gamepad_assembly = 1: PS5 layout  (Triangle/Circle/X/Square on right, D-pad on left)
// - gamepad_button_xy switches between xbox_button_xy[] and ps5_button_xy[]
// - Affects only visual positions, not logical mapping (output indices same)
//
// MAPPING PERSISTENCE:
// - gamepad_mapping[256] saved to disk (binary file, 256 bytes)
// - Loaded on game start (SetGamePadMapping())
// - Updated on drag-drop (inverse tables rebuilt every change)
// - [DATA-CONTRACT:MAPPING] Binary format: 256 bytes, input index → output index
//
// EVENT PROCESSING FLOW:
// 1. [FLOW:INPUT] Platform backend: gpad_button(b, pos) → gamepad.cpp UpdateGamePadButton()
// 2. UpdateGamePadButton(): Lookup gamepad_mapping[b] → output index
// 3. Update gamepad_button_output[output], return to caller
// 4. Game: Reads gamepad_button_output[0] to check if A pressed
//
// KEY DATA STRUCTURES:
// - gamepad_mapping[256]:      Input → Output mapping table
// - button_mapping[15]:        Output button → input list (inverse)
// - axis_mapping[6]:           Output axis → input list (inverse)
// - gamepad_sprite:            Sprite* for gamepad visual (loaded from .xp file)
// - gamepad_assembly:          0 = Xbox, 1 = PS5
//
// KEY FUNCTIONS:
// - ConnectGamePad():          Called on gamepad mount (LoopInterface::gpad_mount)
// - DisconnectGamePad():       Called on gamepad unmount
// - SetGamePadMapping():       Load mapping from disk
// - GetGamePadMapping():       Return current mapping (for save)
// - UpdateGamePadButton():     Apply mapping to button event, return output indices
// - UpdateGamePadAxis():       Apply mapping to axis event, return output indices
// - PaintGamePad():            Render visual UI (gamepad sprite, drag feedback)
// - GamePadContact():          Handle mouse/touch input for drag-drop
// - GamePadKeyb():             Handle keyboard input (arrow keys, enter, esc)
//
// INTEGRATION POINTS:
// - platform.h LoopInterface   — gpad_mount, gpad_button, gpad_axis callbacks
// - sdl.cpp                    — Calls ConnectGamePad on SDL_CONTROLLERDEVICEADDED
// - game.cpp                   — Reads gamepad_button_output[], gamepad_axis_output[]
//
// KEY FILES:
// - gamepad.cpp        — This file: mapping system, visual UI, event processing
// - gamepad.h          — Public API (LoadGamePad, SetGamePadMapping, etc.)
// - platform.h         — LoopInterface contract (gpad_mount, gpad_button, gpad_axis)
// - sdl.cpp            — SDL_GameController integration
// =============================================================================
...
// INVERSE MAPS — DEPENDENCY LOOKUPS (0xFF terminated)
// WHY inverse tables:
// Inverse tables enable efficient "which inputs map to this output?" queries.
// Example: button_mapping[0] = {5, 12, 0xFF} means input buttons 5 and 12 both map to output A.
// Use case: Visual UI highlights ALL input slots that map to selected output.
//
// REBUILD PROTOCOL (see InvertMap function):
// 1. Clear all existing arrays (free + NULL)
// 2. Count length needed for each output (scan gamepad_mapping[])
// 3. Allocate new arrays with +1 for 0xFF sentinel
// 4. Populate arrays by iterating gamepad_mapping[] again
// 5. Terminate each array with 0xFF sentinel
//
// WHY 0xFF terminator:
// - Variable length: different outputs may have 1, 2, or 10+ inputs mapped
// - Sentinel: 0xFF marks end of list (simplifies iteration without storing lengths)
// - 0xFF is invalid output index (valid range 0-20 for axes+buttons)
static uint8_t* button_mapping[15] = {0};  // Output button → list of input indices (0xFF terminated)
static uint8_t* axis_mapping[6] = {0};     // Output axis → list of input indices (0xFF terminated)

// CURRENTLY DEFINED MAPPING (input → output)
// WHY 256 indices:
// - SDL supports up to 256 buttons/axes (theoretical max, SDL_GameController API limit)
// - 0xFF = unmapped (allows 0-254 as valid output indices)
// - Axes use TWO indices: negative half (2*a) and positive half (2*a+1)
//
// WHY separate mount_mapping vs gamepad_mapping:
// - mount_mapping: saved at gamepad connect time (SDL's default mapping)
// - gamepad_mapping: user-customized mapping (persisted to disk)
// - Allows "reset to default" by copying mount_mapping back to gamepad_mapping
static uint8_t gamepad_mount_mapping[256] = { 0 };  // Mount-time mapping (from SDL)
static uint8_t gamepad_mapping[256] = {0};          // Active mapping (user-configured)
...
// WHY two layouts share same output indices:
// - Logical abstraction independent of visual representation
// - Output index 6 = "A" button on Xbox = "Cross" on PS5 (same game action)
// - Only visual positions differ (gamepad_button_xy[] coordinates change)
// - Mapping persistence portable between layouts (saved mapping works on both)
...
// [FLOW:INPUT] Gamepad axis event → mapping → output
// Called by platform backend (sdl.cpp) when gamepad axis state changes.
// Translates raw axis index to mapped output index(es).
// WHY axes are more complex than buttons:
// - Axes are bipolar (-32768 to +32767), buttons are unipolar (0 to 32767)
// - Each axis splits into TWO inputs: negative half (index 2*a), positive half (index 2*a+1)
// - Allows independent mapping: left stick X-axis negative → "move left", positive → "move right"
...
// [FLOW:INPUT] Gamepad button event → mapping → output
// Called by platform backend (sdl.cpp) when gamepad button state changes.
// Translates raw button index to mapped output index(es).
int UpdateGamePadButton(int b, int16_t v, uint32_t out[1])
{
	...
	if (gamepad_input[2*gamepad_axes + b] != pos)
	{
		...
		// WHY offset by 2*gamepad_axes:
		// - Axes use TWO indices each (negative half, positive half)
		// - Buttons start AFTER all axis indices
		// - Example: 6 axes → indices 0-11 for axes, 12+ for buttons
...
// WHY rebuild inverse tables from scratch (not incremental update):
// - Simpler: No need to track old→new delta, just regenerate entire structure
// - Safer: No risk of stale pointers or incomplete updates (old arrays freed before rebuild)
// - Fast enough: 256 iterations is O(1) for human interaction (drag-drop latency < 1ms)
//
// REBUILD PROTOCOL:
// 1. Count length needed for each output (first pass over gamepad_mapping[])
// 2. Allocate new arrays with +1 for 0xFF sentinel
// 3. Populate arrays (second pass over gamepad_mapping[])
// 4. Terminate each array with 0xFF sentinel
//
// WHY 0xFF sentinel:
// - Marks end of variable-length list (no separate length storage needed)
// - 0xFF is invalid output index (valid range 0-20)
// - Simplifies iteration: while (button_mapping[i][j] != 0xFF) { ... }
...
// [FLOW:INPUT] Gamepad connected, mapping applied
// Called by platform backend (sdl.cpp) via LoopInterface::gpad_mount when gamepad is detected.
// Initializes mapping state and rebuilds inverse lookup tables.
void ConnectGamePad(const char* name, int axes, int buttons, const uint8_t mapping[])
{
	...
	// WHY 2*axes+buttons:
	// - Each axis splits into TWO indices (negative half, positive half)
	// - Buttons are single indices
	// - Total input indices = 2*axes + buttons
	...
	{
		// WHY 0xFF for unmapped:
		// - 0xFF is invalid output index (valid range 0-20)
		// - Allows detection of unmapped inputs during event processing
	...
	// WHY rebuild inverse tables here:
	// - New gamepad may have different button/axis count than previous
	// - Inverse tables must match current gamepad_mapping[] state
	InvertMap(mappings);
...
// [DATA-CONTRACT:MAPPING] Load mapping from disk (256 bytes binary)
// WHY rebuild inverse tables after loading:
// - Old inverse tables point to previous mapping state (stale pointers)
// - Must regenerate to reflect new gamepad_mapping[] values
// - InvertMap() frees old arrays and allocates new ones
void SetGamePadMapping(const uint8_t* map)
{
	...
	// WHY free old inverse tables before InvertMap:
	// - InvertMap() will allocate new arrays
	// - Old arrays must be freed to avoid memory leak
	// - InvertMap() does NOT free old arrays (it expects them to be NULL or valid)
	for (int i=0; i<6; i++)
	{
		if (axis_mapping[i])
		{
...
// WHY contact tracking with gamepad_contact_from/pos/output/ui:
// - Drag-drop UI requires tracking: what is being dragged, from where, to where
// - gamepad_contact_from[2]: drag start position (cell coordinates)
// - gamepad_contact_pos[2]:  current drag position (updated on move)
// - gamepad_contact_output:  output index being dragged (0xFF = input slot)
// - gamepad_contact_ui:      UI element being dragged (0=clear, 1=init, 2=quit)
//
