// Water System — water surface rendering and physics interaction
...
//
// This file contains architectural planning for a future water rendering system
// with cascaded reflections, sprite deformation in mirrored view, and spatial queries.

// DESIGN GOAL: Water as sparse low-poly mesh overlay on terrain
...
// PROPOSED RENDERING PIPELINE (3-pass):
// WHY: Water reflections require rendering the scene twice (once for reflection, once for direct view)
// with depth-testing to avoid wasted work on occluded water triangles.
//
...
// PREREQUISITE: Object instancing system must exist before water can be implemented
// (water reflections need to reflect both terrain AND objects)
...
// REFLECTION MATHEMATICS: Transform points across a water plane
//
// WHY: Each water triangle acts as a mirror — need to transform geometry to reflection space
// for correct rendering of mirrored terrain/objects beneath water surface.
/*
CASCADED REFLECTION:
...
// SPATIAL QUERY DESIGN: Fast geometric queries for water interaction
//
// WHY: Need efficient ray-casting (player looking at water surface) and
// frustum culling (what geometry needs reflection rendering) for acceptable performance.
//
// Required queries:
